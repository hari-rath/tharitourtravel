...
<div class="container mx-auto p-6">
    ...
    <div class="flex justify-between">
        <h1 class="text-2xl font-extrabold mb-6">Category List</h1>
    ...
    <table class="w-full bg-white shadow rounded-lg overflow-hidden">
        ...
        <tbody>
            <?php foreach ($categoryDetails as $index => $categoryDetail): ?>
                <tr class="border-b">
                    <td class="p-3"><?= $index + 1; ?></td> <!-- Serial number -->
                    <td class="p-3"><?= esc($categoryDetail['name']); ?></td>
                    <td class="p-3">
                        <a href="<?= base_url('admin/category/toggle-status/' . $categoryDetail['id']); ?>"
                           class="<?= $categoryDetail['status'] == 1 ? 'text-green-600' : 'text-red-500'; ?> hover:underline"
                           title="Click to change status"
                           onclick="return confirm('Change status of this Cagetory?')">
                            <?= $categoryDetail['status'] == 1 ? 'Active' : 'Inactive'; ?>
                        </a>
                    </td>
                    <td class="p-3 space-x-4">
                        <a href="<?= base_url('admin/category/edit/' . $categoryDetail['id']); ?>" class="text-blue-600 hover:text-blue-800" title="Edit">
                            <i class="fas fa-edit"></i>
                        </a>
                        <a href="<?= base_url('admin/category/delete/' . $categoryDetail['id']); ?>" class="text-red-600 hover:text-red-800" title="Delete" onclick="return confirm('Are you sure?')">
